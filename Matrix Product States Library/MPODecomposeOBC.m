function [MPOArray, operatorNorm] = MPODecomposeOBC(operatorVector,dVector)
...
% operatorNorm returns the norm of the input state; MPO is normalized????
...
for i = 1:1:L
    ...
    % Now calculate the rank of c0, which gives us the next xdims:    
    %rankMatrix = rank(c0);
    
%     SizeRef = size(c0);
%     
%     if i ~= L
%         d_iplus1 = dVector(i+1);
%         rankMatrix = SizeRef(1)*SizeRef(2) / (di^2 * (dtot / d_iplus1)^2);
%     else
%         rankMatrix = SizeRef(1)*SizeRef(2);
%     end
%     
    %rankMatrix = SizeRef(1)*SizeRef(2) / (d*(d^(L-(i+1))));
